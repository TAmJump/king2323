...
were thought to still be on the legacy `.lang-en` / `.lang-ja` pattern
turned out to be in better shape than the session ⑪ handoff suggested:

- `entry.html` and `how-it-works.html` were already 100 % on the
  `data-i18n-html` system with all 27 keys translated into all 9
  non-English TIER 1 languages. No HTML changes needed for them.
- `entry.html` had only **two** keys missing in the dictionary
  (`entry.hint.payment_email`, `entry.label.card`); added.

The two pages that did still carry the bilingual span pattern were the
two **legal pages** — `rules.html` (16 pairs) and `risk.html`
(14 pairs). Both are now migrated.

## Approach for legal pages (deliberate)

Legal-substance paragraphs are NOT translated into the 8 additional
languages. They remain `<p>` (English) + `<p class="jp">` (Japanese),
relying on the existing `main.css` rule that hides one or the other
based on `[data-display-lang]`. Machine-translating legal text into
Korean / Spanish / Hindi / Vietnamese / Portuguese / Indonesian /
Thai / French without expert legal review would be a liability —
participants outside JP read the authoritative English version.

What IS translated to 10 languages: section headings, the corporate
footer label, the disclaimer meta-line, and the site-wide footer
links — every navigational anchor a non-JP user needs to orient on
the page. This is the right division of labor between machine
translation and legal precision.

## What changed

1. **`I18N_CONTENT` extensions (`js/i18n.js`):**
   - `entry.hint.payment_email`, `entry.label.card` — fills two
     gaps the entry form had been falling back to the JP source on.
   - `corp.label` — corporate footer label used by every legal page.
   - 12 × `rulespage.*` keys: `title` + 11 section headings
     (`schedule`, `entry`, `what_is`, `mission_entry`, `verification`,
     `selection`, `review`, `opening`, `cancellation`, `geographic`).
   - 9 × `riskpage.*` keys: `title` + 8 section headings
     (`read_before`, `what_you_receive`, `founding_is_opening`,
     `what_this_is_not`, `geo_limit`, `taxes`, `do_not_if`, `contact`).
   - Every new key in all 10 TIER 1 languages.
   - Namespaced as `rulespage.*` / `riskpage.*` to avoid colliding
     with the existing `rules.*` block, which serves the homepage
     Edge-Rules section, not the legal page.

2. **`rules.html` migration:**
   - 11 `<h1>` / `<h2>` bilingual-span pairs → single elements with
     `data-i18n-html="rulespage.h.*"`.
   - 1 corporate footer label pair → `data-i18n-html="corp.label"`.
   - 3 corporate-link pairs (Terms / Privacy / Commercial
     Transactions) → existing `footer.terms` / `footer.privacy` /
     `footer.commerce` keys.
   - 1 disclaimer meta-line pair → `footer.disclaimer` key.
   - Total: 16 span pairs retired, 0 remaining.
   - Cache buster bumped to `?v=20260523k`.

3. **`risk.html` migration (same pattern):**
   - 9 heading pairs → `riskpage.h.*` keys.
   - 5 footer pairs (corp label + 3 corporate links + disclaimer) →
     existing or new shared footer keys.
   - Total: 14 span pairs retired, 0 remaining.
   - Cache buster bumped to `?v=20260523k`.

## Pages explicitly left alone (with rationale)

- **`money.html`** — long-form marketing doctrine page, English-only
  body. Migrating its 14 chapter headings to 10 languages would
  triple the page's translation surface for marginal user benefit
  (the page reads coherently in English; non-JP users today already
  see English-only content for the whole body). Out of scope for
  this commit.
- **`verify.html`** — auth-gated operator tool. Body content is
  generated by JS at runtime against the worker API. Strings there
  are status messages, not static content. Migration would mean
  introducing a `VERIFY_I18N` per-page dict; not justified for an
  operator-only surface.

## Verification

```
grep -cE 'class="lang-en"|class="lang-ja"' rules.html   →  0
grep -cE 'class="lang-en"|class="lang-ja"' risk.html    →  0
grep -c 'data-i18n-html='                  rules.html   →  22
grep -c 'data-i18n-html='                  risk.html    →  20
node -e "new Function(<i18n.js>)"                       →  OK (270749 chars)
```

After this commit, the legacy bilingual-span pattern survives in
`money.html` (chapter titles), and as a handful of stray pairs in
some `index.html` sections — sites of explicit "out of scope" decisions
documented above and in the session ⑪ handoff. Every page a
non-JP entry-funnel user would actually traverse (`index.html`,
`how-it-works.html`, `entry.html`, `mypage.html`, `play.html`,
`kings.html`, `preview.html`, `rules.html`, `risk.html`,
`verify.html`'s chrome) now renders in all 10 TIER 1 languages.
